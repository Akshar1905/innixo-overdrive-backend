...
export async function registerRoutes(
  ...
  const getEventFee = (eventName: string): number => {
    const event = eventsConfig[eventName as keyof typeof eventsConfig];
  ...
  app.post("/api/payments/create-order", strictLimiter, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // Return valid error
        res.status(500).json({ message: "Payment initialization failed", error: (rpError as any).error });
      }
